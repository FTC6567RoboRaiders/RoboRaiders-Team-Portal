...
export interface StorageLocationModalProps {
  ...
  initialEditingLocId?: string | null;
...
export const StorageLocationModal: React.FC<StorageLocationModalProps> = ({
  ...
  // Tab state: 'list' (manage/edit) vs 'create'
  ...
  // Auto-switch to edit mode if initialEditingLocId is passed
  useEffect(() => {
    if (isOpen && initialEditingLocId) {
      const target = locations.find(l => l.id === initialEditingLocId);
      if (target) {
        handleStartEdit(target);
        setActiveTab('list');
      }
    } else if (isOpen && locations.length === 0) {
      setActiveTab('create');
    }
  }, [isOpen, initialEditingLocId, locations]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      ...
      >
        ...
        {/* Navigation Tabs */}
        <div className="px-6 pt-2.5 pb-0 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-850">
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3.5 py-2 text-xs font-bold border-b-2 transition-all flex items-center gap-1.5 cursor-pointer ${
                activeTab === 'list'
                  ? 'border-rose-500 text-rose-600 dark:text-rose-400 bg-white dark:bg-slate-900 rounded-t-lg'
                  : 'border-transparent text-slate-500 hover:text-slate-800 dark:text-slate-400'
              }`}
            >
              <Layers className="w-3.5 h-3.5" />
              <span>Locations &amp; Edit ({locations.length})</span>
            </button>
          ...
          {activeTab === 'create' && (
            ...
            <div className="flex justify-end gap-2 pt-1">
              <button
                type="button"
                onClick={() => setActiveTab('list')}
                className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
            ...
            {locations.length === 0 ? (
              <div className="p-8 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5 mb-3">
                  Add your first workshop bin, cabinet, or lab drawer with sub-areas.
                </p>
                <button
                  type="button"
                  onClick={() => setActiveTab('create')}
                  className="px-3.5 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs inline-flex items-center gap-1.5 shadow-xs cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
