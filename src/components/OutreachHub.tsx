...
export default function OutreachHub({ 
  ...
  const [lightboxImageUrl, setLightboxImageUrl] = useState<{ images: OutreachImage[], currentIndex: number } | null>(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      } else if (e.key === 'ArrowRight' && lightboxImageUrl && lightboxImageUrl.images.length > 1) {
        setLightboxImageUrl(prev => prev ? { ...prev, currentIndex: (prev.currentIndex + 1) % prev.images.length } : null);
      } else if (e.key === 'ArrowLeft' && lightboxImageUrl && lightboxImageUrl.images.length > 1) {
        setLightboxImageUrl(prev => prev ? { ...prev, currentIndex: (prev.currentIndex - 1 + prev.images.length) % prev.images.length } : null);
      }
    };
    if (lightboxImageUrl) {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-50 text-slate-900 transition-colors animate-fade-in p-4 md:p-6 dark:bg-slate-800 dark:text-slate-400" id="outreach-dashboard-root">
      
      {/* Toast Alert overlay */}
      {toastMsg && (
        <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2 px-4 py-2.5 rounded-lg shadow-xl border bg-slate-900 text-white border-slate-800 text-xs font-bold animate-fade-in dark:bg-slate-950">
      ...
      {/* HEADER SECTION */}
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-extrabold tracking-tight uppercase font-display text-slate-800 mt-1.5 flex items-center gap-2 dark:text-slate-400">
            <Heart className="w-5 h-5 text-emerald-550 fill-emerald-500" />
            <span>Community Outreach Events Ledger</span>
          </h1>
          <p className="text-xs text-slate-500 mt-1 max-w-2xl dark:text-slate-400">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start xl:self-center">
          {onBack && (
            <button
              onClick={onBack}
              className="bg-slate-200 hover:bg-slate-350 text-slate-800 font-extrabold text-[11px] uppercase tracking-widest px-4 py-2.5 rounded-lg shadow-md transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center gap-1.5 cursor-pointer border border-slate-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        {/* Metric 1 */}
        <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex items-center gap-3 dark:bg-slate-900 dark:border-slate-800">
          <div className="p-3 rounded-lg bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400">
            <Calendar className="w-5 h-5" />
          </div>
          <div>
            <span className="text-[10px] font-mono font-bold tracking-widest uppercase text-slate-400 dark:text-slate-500">Total Events Done</span>
            <p className="text-lg font-black font-display text-slate-800 dark:text-slate-400">{totalEvents}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex items-center gap-3 dark:bg-slate-900 dark:border-slate-800">
          ...
          <div>
            <span className="text-[10px] font-mono font-bold tracking-widest uppercase text-slate-400 dark:text-slate-500">Outreach Hours Devoted</span>
            <p className="text-lg font-black font-display text-slate-800 dark:text-slate-400">{totalHours} hrs</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex items-center gap-3 dark:bg-slate-900 dark:border-slate-800">
          ...
          <div>
            <span className="text-[10px] font-mono font-bold tracking-widest uppercase text-slate-400 dark:text-slate-500">Estimated Reach</span>
            <p className="text-lg font-black font-display text-slate-800 dark:text-slate-400">{totalReach}+ people</p>
            <p className="text-[10px] text-slate-450 font-semibold font-mono leading-tight mt-0.5 dark:text-slate-400">
        ...
        {/* Metric 4 */}
        <div className="bg-white border border-slate-200 p-4 rounded-xl shadow-sm flex flex-col justify-center dark:bg-slate-900 dark:border-slate-800">
          <span className="text-[9px] font-mono font-bold tracking-widest uppercase text-slate-400 pl-1 dark:text-slate-500">Outreach Champions</span>
          <div className="flex flex-wrap gap-1 mt-1 pl-1">
            {getTopParticipants().length === 0 ? (
              <span className="text-[10px] text-slate-400 font-mono dark:text-slate-500">No data logged yet</span>
            ) : (
              getTopParticipants().map(([name, hrs]) => (
                <span 
                  key={name} 
                  title={`${hrs} hours logged`}
                  className="bg-slate-100 border border-slate-200 text-[9px] font-bold px-1.5 py-0.5 rounded text-slate-700 max-w-[90px] truncate dark:bg-slate-800 dark:text-slate-300 dark:border-slate-800"
                >
                  🏆 {name} ({hrs}h)
                </span>
              ))
        ...
        <div className="flex flex-col lg:flex-row gap-3.5 items-center justify-between">
          ...
          <div className="relative w-full lg:max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-slate-50 border border-slate-300 rounded-md text-xs text-slate-900 outline-none focus:ring-1 focus:ring-brand focus:bg-white dark:focus:bg-slate-800 transition-all font-mono dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-1.5 shrink-0 text-xs text-slate-500 dark:text-slate-400">
              ...
              <select
                ...
                className="bg-slate-50 border border-slate-300 rounded px-2.5 py-1 text-xs text-slate-800 outline-none focus:ring-1 focus:ring-brand font-bold dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
              >
      ...
      {filteredEvents.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center p-12 bg-white border border-slate-200 rounded-2xl text-center select-none shadow-sm min-h-[300px] dark:bg-slate-900 dark:border-slate-800">
          ...
          <p className="text-xs text-slate-500 max-w-sm mt-1.5 leading-relaxed dark:text-slate-400">
          ...
          {filteredEvents.map((ev) => (
            ...
            >
              ...
              {ev.images && ev.images.length > 0 ? (
                ...
                <div className="h-10 bg-gradient-to-r from-emerald-50 to-blue-50/50 dark:from-emerald-950/20 dark:to-blue-900/10 border-b border-slate-100 dark:border-slate-800" />
              )}
              ...
              <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                ...
                <div>
                  ...
                  <h3 className="text-sm font-black uppercase tracking-wide text-slate-800 font-display leading-tight mt-3 dark:text-slate-400">
                    {ev.title}
                  </h3>
                  ...
                  {/* Description text */}
                  <p className="text-[11px] text-slate-500 leading-relaxed mt-2.5 font-sans whitespace-pre-line dark:text-slate-400">
                    {ev.description}
                  </p>
                  ...
                  {ev.impactMetrics && (
                    <div className="mt-4 bg-emerald-50/40 dark:bg-emerald-950/20 border border-emerald-150 dark:border-emerald-900/40 p-3 rounded-xl flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-sans text-slate-650 mt-0.5 leading-snug dark:text-slate-300">
                          {ev.impactMetrics}
                        </p>
                      </div>
                    </div>
                  ...
                  {(ev.reachedChildren !== undefined || ev.reachedAdults !== undefined) && (
                    <div className="mt-3 bg-blue-50/10 dark:bg-blue-950/10 border border-blue-105/20 dark:border-blue-900/20 p-2.5 rounded-xl flex items-center justify-between">
                      ...
                      <div className="text-[10px] font-mono font-bold text-slate-700 flex gap-2 dark:text-slate-300">
                ...
                <div className="pt-3 border-t border-slate-100 flex flex-col gap-3 dark:border-slate-800">
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto py-1">
                      {ev.images.slice(1).map((img, idx) => (
                        <img
                          ...
                          alt="Sub proof"
                          referrerPolicy="no-referrer"
                          className="w-10 h-10 rounded-md object-cover hover:scale-105 transition-all cursor-zoom-in border border-slate-200 shrink-0 dark:border-slate-800"
                          onClick={() => setLightboxImageUrl({ images: ev.images, currentIndex: idx + 1 })}
                        />
                      ))}
                    </div>
                  )}
                  ...
                  <div>
                    <span className="text-[8.5px] font-mono font-bold text-slate-400 uppercase tracking-widest block mb-1 dark:text-slate-500">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {ev.participants.map((p, idx) => (
                        <span 
                          key={idx} 
                          className="bg-slate-100 text-slate-700 border border-slate-150 text-[9px] font-semibold px-2 py-0.5 rounded flex items-center gap-1 dark:bg-slate-800 dark:text-slate-300"
                        >
              ...
              {/* Bottom control bar */}
              <div className="bg-slate-50 px-5 py-3 border-t border-slate-100 flex justify-between items-center text-[9px] font-mono text-slate-400 dark:bg-slate-800 dark:text-slate-500 dark:border-slate-800">
                <span>Author: {ev.updatedBy}</span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => openEditModal(ev)}
                    title="Edit record parameters"
                    className="p-1 hover:bg-slate-200 rounded text-slate-500 hover:text-emerald-600 dark:hover:text-emerald-450 transition-all cursor-pointer dark:text-slate-400 dark:hover:bg-slate-600"
                  >
                  ...
                  <button
                    ...
                    title="Purge record"
                    className="p-1 hover:bg-red-50 dark:hover:bg-red-950/20 rounded text-slate-500 hover:text-red-500 transition-all cursor-pointer dark:text-slate-400"
                  >
      ...
      {lightboxImageUrl && (
        <div 
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur-xs cursor-zoom-out animate-fade-in group/lightbox"
          onClick={() => setLightboxImageUrl(null)}
        >
          <button 
            className="absolute top-4 right-4 bg-white/10 hover:bg-white/20 text-white rounded-full p-2 border border-white/10 z-10"
            onClick={() => setLightboxImageUrl(null)}
          >
            <X className="w-5 h-5" />
          </button>
          
          {lightboxImageUrl.images.length > 1 && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                setLightboxImageUrl(prev => prev ? { ...prev, currentIndex: (prev.currentIndex - 1 + prev.images.length) % prev.images.length } : null);
              }}
              className="absolute left-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-all md:opacity-0 md:group-hover/lightbox:opacity-100 z-10 border border-white/10"
          ...
          >
            <img 
              key={lightboxImageUrl.images[lightboxImageUrl.currentIndex].dataUrl}
              src={lightboxImageUrl.images[lightboxImageUrl.currentIndex].dataUrl} 
              alt="Expanded view proof" 
              referrerPolicy="no-referrer"
              className="max-w-full max-h-[85vh] rounded-lg shadow-2xl object-contain animate-scale-up" 
            />
            {lightboxImageUrl.images.length > 1 && (
              <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-white/70 text-xs font-mono bg-black/50 px-2 py-1 rounded">
                {lightboxImageUrl.currentIndex + 1} / {lightboxImageUrl.images.length}
              </div>
            )}
          </div>
          ...
          {lightboxImageUrl.images.length > 1 && (
            <button
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-all md:opacity-0 md:group-hover/lightbox:opacity-100 z-10 border border-white/10"
              title="Next image"
            >
      ...
      {isExportModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-scale-up text-slate-800 p-0 dark:bg-slate-900 dark:text-slate-400 dark:border-slate-800">
            {/* Header */}
            <div className="px-4 py-3.5 border-b border-slate-150 flex justify-between items-center bg-slate-900 text-white dark:bg-slate-950">
              <div className="flex items-center gap-2">
                <Printer className="w-4 h-4 text-emerald-400" />
            ...
            <div className="p-5 flex flex-col gap-4">
              <p className="text-xs leading-normal font-medium text-slate-650 dark:text-slate-300">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`px-3 py-3 rounded-lg border text-xs font-bold transition-all flex flex-col items-center justify-center gap-1.5 cursor-pointer ${
                      exportScope === 'all'
                        ...
                        : 'border-slate-200 dark:border-slate-805 hover:bg-slate-50 dark:hover:bg-slate-850 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-3 rounded-lg border text-xs font-bold transition-all flex flex-col items-center justify-center gap-1.5 cursor-pointer ${
                      filteredEvents.length === 0 
                        ? 'opacity-50 cursor-not-allowed border-slate-100 bg-slate-50 dark:border-slate-800 dark:bg-slate-800 text-slate-450'
              ...
              <div className="bg-slate-100 p-3.5 rounded-lg border border-slate-200 flex flex-col gap-1.5 dark:bg-slate-800 dark:border-slate-800">
                <div className="flex justify-between items-center text-xs font-mono">
                  <span className="text-slate-500 uppercase font-bold text-[9px] tracking-wider dark:text-slate-400">Events Compiled:</span>
                  <span className="font-extrabold text-slate-950 dark:text-slate-400">
                    {exportScope === 'all' ? events.length : filteredEvents.length} Records
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs font-mono">
                  <span className="text-slate-500 uppercase font-bold text-[9px] tracking-wider dark:text-slate-400">Total Combined Hours:</span>
                  <span className="font-extrabold text-emerald-600 dark:text-emerald-400">
                    {(exportScope === 'all' ? events : filteredEvents).reduce((sum, ev) => sum + (ev.hoursLogged || 0), 0)} Hours
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs font-mono">
                  <span className="text-slate-500 uppercase font-bold text-[9px] tracking-wider dark:text-slate-400">Total Estimated Reach:</span>
                ...
                <p className="text-[9px] text-slate-400 font-mono leading-normal mt-1 border-t border-slate-200 pt-1.5 dark:text-slate-500 dark:border-slate-800">
              ...
              <div className="flex gap-2 justify-end pt-2">
                <button
                  type="button"
                  onClick={() => setIsExportModalOpen(false)}
                  className="px-4 py-2 text-xs font-bold font-mono uppercase bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors cursor-pointer dark:bg-slate-800 dark:hover:bg-slate-600"
                >
                  Cancel
                </button>
                <button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-xl overflow-hidden shadow-2xl animate-scale-up dark:bg-slate-900 dark:border-slate-800">
            
            <div className="p-4 md:p-5 border-b border-slate-150 flex justify-between items-center bg-slate-50 dark:bg-slate-800">
              <h3 className="font-extrabold uppercase text-xs md:text-sm tracking-widest font-display text-slate-800 flex items-center gap-2 dark:text-slate-400">
              ...
              <button 
                ...
                className="text-slate-400 hover:text-slate-800 text-xs font-mono font-bold hover:bg-slate-100 px-2 py-1 rounded transition-colors cursor-pointer dark:text-slate-500 dark:hover:bg-slate-700 dark:hover:text-slate-100"
              >
                Close (ESC)
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-300 rounded-md px-3 py-1.5 text-xs text-slate-900 outline-none focus:ring-1 focus:ring-emerald-500 focus:bg-white dark:focus:bg-slate-800 transition-all font-bold dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-md px-3 py-1.5 text-xs text-slate-900 outline-none focus:ring-1 focus:ring-emerald-500 transition-all font-mono font-bold dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
                  />
                </div>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-300 rounded-md px-3 py-1.5 text-xs text-slate-900 outline-none focus:ring-1 focus:ring-emerald-500 transition-all font-bold dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
                  />
                </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-slate-50 border border-slate-300 rounded-md px-3 py-1.5 text-xs text-slate-900 outline-none focus:ring-1 focus:ring-emerald-500 focus:bg-white dark:focus:bg-slate-800 transition-all font-sans leading-relaxed dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
                />
              </div>
              ...
              <div className="flex flex-col gap-1">
                <label className="text-[9px] font-mono font-black text-slate-400 uppercase tracking-widest block dark:text-slate-500">
                  Select Attending Participants <span className="text-emerald-500">*</span>
                </label>
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 max-h-32 overflow-y-auto dark:bg-slate-800 dark:border-slate-800">
                        ...
                        <button
                          ...
                          className={`px-2 py-1 rounded text-left text-[11px] truncate flex items-center gap-1.5 border transition-all ${
                            isSelected 
                              ...
                              : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-800 text-slate-650 dark:text-slate-400'
                          }`}
                        >
                ...
                <div className="flex items-center gap-2 mt-2">
                  <input
                    ...
                    className="flex-1 bg-slate-50 border border-slate-300 rounded px-2.5 py-1 text-xs text-slate-900 outline-none focus:ring-1 focus:ring-emerald-500 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-800"
                  />
                  <button
                    type="button"
                    onClick={handleAddCustomParticipant}
                    className="bg-slate-200 hover:bg-slate-300 text-slate-800 text-xs px-3 py-1 rounded font-bold transition-all shrink-0 uppercase dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-500"
                  >
                    Add Label
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all flex flex-col items-center justify-center gap-1.5 ${
                    isDragging 
                      ...
                      : 'border-slate-300 dark:border-slate-800 hover:border-emerald-400/50 hover:bg-slate-50/50 dark:hover:bg-slate-500/50'
                  }`}
                >
                  <input
                    type="file"
                    ...
                    className="hidden"
                  />
                  <Upload className="w-6 h-6 text-slate-400 mb-1 dark:text-slate-500" />
                  <span className="text-xs font-bold text-slate-700 dark:text-slate-300">
                    Drag and drop photos here, or click to upload
                  </span>
                  <p className="text-[9px] text-slate-450 leading-normal dark:text-slate-400">
                ...
                {eventImages.length > 0 && (
                  <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 mt-2.5 bg-slate-50 p-2.5 rounded-xl border border-slate-100 dark:bg-slate-800 dark:border-slate-800">
                    {eventImages.map(img => (
                      <div key={img.id} className="relative group rounded-lg overflow-hidden border border-slate-200 aspect-square dark:border-slate-800">
                        <img
                          src={img.dataUrl}
                          alt="preview"
                          referrerPolicy="no-referrer"
              ...
              {/* Form Buttons */}
              <div className="flex items-center justify-end gap-2 pt-4 mt-2 border-t border-slate-150">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="bg-slate-105 hover:bg-slate-200 text-slate-700 font-bold px-4 py-2.5 rounded text-xs uppercase cursor-pointer dark:text-slate-300 dark:hover:bg-slate-600"
                >
                  Cancel
                </button>
                <button
